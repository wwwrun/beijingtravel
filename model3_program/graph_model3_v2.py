...
class GraphForEntityV0_4_9():
                ...
                if orig_word == seed:  # 如果该词与seed一样，且是个动词，因为动词里可能有’#‘
                    ...
                    for aIndex in range(sentence.index(item) - 1,-1,-1):  # 再次遍历在sentence中该动词前的item
                        ...
                        if headFirstVerb is False and '_n' in aWord and aNumlocation==-1:
                            if item[1] - aItem[1] - len(orig_word) <= 5 and len(clean_word)>1:
                                find_nh=True
                                nh_clean_word=clean_word

                        if headFirstVerb is False and (clean_word in allEntityList) and aItem[1] < item[1] and aNumlocation != -1:
                            # 设置这个共现实体要在种子的前面 （头实体）
                            if len(clean_word)>1 and item[1] - aItem[1] - len(clean_word) <= 5 and '_v' not in aWord:  # 设置两个实体距离不能超过10
                    ...
                    if headNode is not None and tailNode is not None and headNode.lemma[:headNode.lemma.rfind('_')]!=\
                                               tailNode.lemma[:tailNode.lemma.rfind('_')]:
        ...
        for entityNode in nodeList:
            if entityNode.type == "entity":
                if "head" in entityNode.lemma:
                    ...
                    if headStrAllType==[""]:
                        edge = Edge(entityNode, Node("其他"+str(index)+"-domain", 0, "type"), 0.5)
                        ...
                        if hasType is False:
                            for type in headStrAllType:
                                if type!="":
                                    edge = Edge(entityNode, Node(type + "-domain", 0, "type"), 0.5)
                                    self.Graph.append(edge)
                ...
                if "tail" in entityNode.lemma:
                    ...
                    if tailStrAllType==[""]:
                        edge = Edge(entityNode, Node("其他"+str(index)+"-range", 0, "type"), 0.5)
                        self.Graph.append(edge)
                        self.entity_type_list.append([edge.nodeA.lemma, edge.nodeB.lemma,10])
                    else:
                        ...
                        if hasType is False:
                            for type in tailStrAllType:
                                if type!="":
                                    edge = Edge(entityNode, Node(type + "-range", 0, "type"), 0.5)
                                    self.Graph.append(edge)
                                    self.entity_type_list.append([edge.nodeA.lemma, edge.nodeB.lemma, 10])
            index+=1
       ...
       """
        for i in range(len(domainNodeList)):
            ...
            for j in range(i + 1, len(domainNodeList)):
                ...
                # if '其他' in hNodeStr or '其他' in tNodeStr:
    ...
    def normalization(self,typeDict,flag):
        ...
        if flag:
            ...
            for type in typeDict:
                if 'domain' in type[0]:
                    # 临时改为不归一化
                    newDomainTypeDict[type[0]] = type[1]
                ...
                if 'range' in type[0]:
                    newRangeTypeDict[type[0]] = type[1]
    ...
    def get_result(self):
        ...
        self.nodeDict = dict()
        # print(nodeScoreDict)
        # 对分值进行归一化，改为domain、range分值不归一化，头尾实体和关系分值归一化
        newDomainTypeDict, newRangeTypeDict = self.normalization(typeDict,True)
        entity_score_list=entityDict
        relation_score_list=relationDict
...
if __name__ == '__main__':
   ...
   # wv_from_text = gensim.models.KeyedVectors.load(
    ...
    f = open(r'C:\Users\wtsin\PycharmProjects\beijingtravel\all_entity_12766.json', 'r', encoding="utf-8")  # 所有实体
    file = f.read()
    all_entity = json.loads(file)
    f.close()

    #with open(r'./entity_verb_result/subject_ev_all_file_0527.json','r',encoding='utf-8')as f:
    with open(r'C:\Users\wtsin\PycharmProjects\beijingtravel\entity_verb_result\subject_ev_yiheyuan2.json', 'r', encoding='utf-8')as f:
    ...
    #     sentence=All_sentence_dict[k]
        ...
        # 开始生成单个文档的图结构
        g1 = GraphForEntityV0_4_9(wv_from_text,sentence_dict,all_entity)
        ...
        with open(r'./relation_domain_range_v2-1.json','w+',encoding='utf-8')as f:
        ...
        for tri in g1.triple_list:
            ...
            a=0.8
            tri_score = math.log(
                a*(domain_type_score + range_type_score)+(1-a)*0.0001*(head_score +relation_score+tail_score) + 0.5)
    ...
    print('All 文档处理完毕')
    with open('./model3_result_yiheyuan迭代3_v2-1.json','w+',encoding='utf-8')as f:
